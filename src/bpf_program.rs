...
fn attach_uprobes(skel: &mut BpfcontainSkel) -> Result<()> {
    ...
    // RUNC INTEGARTION STUFF BEGINS

    // probe attached to https://github.com/opencontainers/runc/blob/b802def2f617eeed5c0a21da131e96a6b6095d95/libcontainer/container_linux.go#L245
    let runc_exec = "github.com/opencontainers/runc/libcontainer.(*linuxContainer).exec";
    skel.links.runc_exec_enter = skel.progs_mut().runc_exec_enter().attach_uprobe_symbol(false,-1,&Path::new(runc_binary_path),runc_exec)?.into();

    // probe attached to https://github.com/opencontainers/runc/blob/b802def2f617eeed5c0a21da131e96a6b6095d95/libcontainer/process_linux.go#L310
    let runc_get_init_process_pid = "github.com/opencontainers/runc/libcontainer.(*initProcess).pid";
    skel.links.runc_get_init_process_pid_enter = skel.progs_mut().runc_get_init_process_pid_enter().attach_uprobe_symbol(false,-1,&Path::new(runc_binary_path),runc_get_init_process_pid)?.into();
